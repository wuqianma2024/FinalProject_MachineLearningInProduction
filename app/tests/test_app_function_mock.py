# Mock testing and numpy, pandas imports for unit tests.
...
def test_initialize_tfidf_vectorizer():
    # Prepare a small corpus to test TF-IDF vectorizer initialization and matrix generation.
    corpus = ["book one summary", "book two summary"]
    vectorizer, matrix = initialize_tfidf_vectorizer(corpus)
    
    # Ensure the vectorizer and the generated matrix are not None and have expected dimensions.
...
def test_find_similar_books_summary(mock_transform, mock_cosine_similarity):
    # Mock the transform and cosine similarity methods to test finding similar books.
    ...
    # Simulate finding top 3 similar books based on a given summary.
    ...
    # Validate that the correct number of similar books are found and have expected titles.
...
def test_recommend_books_genre(mock_load_mlb_instance, mock_load_knn_model):
    # Mock responses for loading the KNN model and MultiLabelBinarizer instance.
    ...
    # Simulate recommending books based on genre using the mocked model and MLB instance.
    df = pd.DataFrame({
        'Title': ['Book 1', 'Book 2', 'Book 3', 'Book 4', 'Book 5'],
